...
async function onSubmit() {
  ...
  try {
    ...
    if (resp && resp.data && resp.data.access_token) {
      ...
      // navigate directly to dashboard so the root redirect to landing is avoided
...
.column-block { padding: 0.65rem 0; border-top: 1px solid rgba(0,0,0,0.06); margin-top: 0.5rem; font-weight: 700; }
.subhead { color: #374151; letter-spacing: 0.12em; }
@media (min-width: 768px) {
  .paper-root { background-image: linear-gradient(rgba(0,0,0,0.02) 1px, transparent 1px); background-size: 100% 26px; }
}

/* Landing-like masthead styles */
